...
function getUnratedProductsOfUser($userID, $itemID){
	$dbCon = new databaseManager();
	$query="select distinct(userID) as user from rating where and itemID=? and ratingValue is null";
...
function predictToAllUsers(){
	...
	for($i=0;$i<count($p);$i++){
		...
		for($j=0;$j<count($users);$j++){
			echo "Prediction for product $p[$i] to user $users[$j] is: ";print_r(predictProduct($p[$i], $users[$j]));echo "<br>";
		}
	}
}
predictToAllUsers();
...
echo "Rating Prediction of item: 84 for user: 52 is:  ";print_r(predictProduct(12, 52));echo "<br>";
